...
class EvaluationDatasetInterHuman(Dataset):
    ...
    """
    def __init__(self, model, dataset, device, mm_num_samples, mm_num_repeats, llm, normalize):
        """
        ...
        if llm:
            ...
            system_template = """
                                Having the description of an interaction, extract individual descriptions for the motions of each individual.
                                ---
                                Interaction Description: In an intense boxing match, one person attacks the opponent with straight
                                punch, and then the opponent falls over.
                                ...
                                —--
                            """
            user_template = "Interaction Description: {interaction_description}"
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader)):
                ...
                if self.extended:
                    ...
                    if llm: 
                        # Generate individual descriptions from the interaction description
                        interaction_description = text
                        llm_output = chain.invoke({"interaction_description": interaction_description})
                        llm_output = llm_output.replace("Individual Motion 1:", "").replace("Individual Motion 2:", "").replace("—--", "").strip()

                        try:
                            text_individual1 = (llm_output.split("\n")[0].strip(),)
                        except:
                            text_individual1 = text

                        try:
                            text_individual2 = (llm_output.split("\n")[1].strip(),)
                        except:
                            text_individual2 = text

                        print("Interaction Description:", interaction_description)
                        print("Individual Motion 1:", text_individual1)
                        print("Individual Motion 2:", text_individual2)
                else:
                ...
                if normalize:
                    ...
                    motions_output = batch["output"].reshape(batch["output"].shape[0], batch["output"].shape[1], 2, -1).cpu().detach().numpy()
...
class EvaluationDatasetDualMDM(Dataset):
    ...
    """
    def __init__(self, model, dataset, device, num_repeats, normalize):
        """
        Initialization of the dataset and generation of the motions.
            :param model: Model to generate the motions.
            :param dataset: Ground truth dataset.
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader)):
                ...
                elif hasattr(self.model, "mixing"):
                    self.model.mixing.force_influence_val = 0
                ...
                if normalize:
                    ...
                    motions_output_interaction = batch_interaction["output"].reshape(batch_interaction["output"].shape[0], batch_interaction["output"].shape[1], 2, -1).cpu().detach().numpy()
                ...
                if hasattr(self.model, "decoder"):
                    ...
                    self.model.mixing.force_influence_val = None
                ...
                if normalize:
                    ...
                    motions_output_individual = batch_individual["output"].reshape(batch_individual["output"].shape[0], batch_individual["output"].shape[1], 2, -1).cpu().detach().numpy()
...
class EvaluationDatasetHumanML3D(Dataset):
    """
    Evaluation Dataset of InterHuman.
    Motions are generated by the trained model to later be compared with the ground truth.
    """
    def __init__(self, model, dataset, device, mm_num_samples, mm_num_repeats, normalize):
        """
        Initialization of the dataset and generation of the motions.
            :param model: Model to generate the motions.
            :param dataset: Ground truth dataset.
...
class MMGeneratedDatasetHumanML3D(Dataset):
    """
    Dataset for the MultiModality metric.
    """
    def __init__(self, motion_dataset):
