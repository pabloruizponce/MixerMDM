...
class LitTrainModel(pl.LightningModule):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        self.it += 1
        if self.it % self.cfg.TRAIN.LOG_STEPS == 0 and self.device.index == 1:
...
if __name__ == '__main__':
    ...
    wandb_logger = WandbLogger(project='finetuned', name=train_cfg.GENERAL.EXP_NAME)
    ...
    model = in2IN(model_cfg, mode="interaction")

    # Now here is the magic
    # We need to load the individual checkpoint into the the model in interacion mode
    ckpt = torch.load(model_cfg.CHECKPOINT, map_location="cpu")

    # The stric mode must be False, because the model has more parameters than the individual model
    # But at the same time is important to check whether the model has the same parameters as the individual model
    # Check which layer are overlapping between the model and the state dict that we are loading
    for k in list(ckpt.keys()):  # Create a list of keys to iterate over
        if "individual" in k:
            print("Swapping individual for interaction ", k)
            ckpt[k.replace("individual", "interaction")] = ckpt.pop(k)

    overlapping_layers = []
    non_overlapping_layers = []
    extra_layers = []

    for k in model.state_dict():
        if k in ckpt.keys(): 
            overlapping_layers.append(k)
        else:
            non_overlapping_layers.append(k)

    for k in ckpt.keys():
        if k not in model.state_dict():
            extra_layers.append(k)

    for k in overlapping_layers:
        print("Overlapping Layers: ", k)
    
    for k in non_overlapping_layers:
        print("Non Overlapping Layers: ", k)

    for k in extra_layers:
        print("Extra Layers: ", k)

    # Load the checkpoint
    model.load_state_dict(ckpt, strict=False)
    print("checkpoint state loaded!")

    # Freeze the layers of self attention (sa)
    for name, param in model.named_parameters():
        if "sa_block" in name:
            param.requires_grad = False

    # Now print the trainable layers of the model
    for name, param in model.named_parameters():
        if param.requires_grad:
            print(name)

    litmodel = LitTrainModel(model, train_cfg, mode="interaction")
