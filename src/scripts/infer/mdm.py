...
class LitGenModel(LightningModule):
    ...
    def generate_one_sample(self, prompt_interaction, name):
        self.model.eval()
        batch = OrderedDict({})
        ...
        result_path = f"{self.save_folder}/{name}_skeleton.mp4"
...
if __name__ == '__main__':
    ...
    if model_cfg.CHECKPOINT:
        ...
        print("State dict")
        for s in list(ckpt["state_dict"].keys()):
            print(s)
        print("___________")
    ...
    litmodel = LitGenModel(model, infer_cfg, args.out, mode="individual").to(torch.device("cuda:"+ args.device))

    for i in range(5):
        litmodel.generate_one_sample(args.text_interaction, args.name + f"_{i}")
