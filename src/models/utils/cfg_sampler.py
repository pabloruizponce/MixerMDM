...
class ClassifierFreeSampleModelX2(nn.Module):
    ...
    def forward(self, x, x2, timesteps, cond=None, mask=None):
        B, T, D = x.shape

        x_combined = torch.cat([x, x], dim=0)
        x2_combined = torch.cat([x2, x2], dim=0)
        ...
        out = self.model(x_combined, timesteps_combined, cond=cond, mask=mask, x2=x2_combined)
