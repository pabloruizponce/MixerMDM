...
from evaluation.utils import get_dataset_motion_loader_hml3d, get_motion_loader_humanml3d, EvaluatorModelWrapperIndividual
...
if __name__ == '__main__':
    ...
    data_cfg = get_config("configs/datasets.yaml").humanml3d_test
    ...
    eval_motion_loaders[model_cfg.NAME] = lambda: get_motion_loader_humanml3d(
                                            batch_size,
                                            model,
                                            gt_dataset,
                                            device,
    ...
    device = torch.device('cuda:%d' % args.device if torch.cuda.is_available() else 'cpu')
    gt_loader, gt_dataset = get_dataset_motion_loader_hml3d(data_cfg, batch_size, num_samples)
    evalmodel_cfg = get_config(args.evaluator)
    eval_wrapper = EvaluatorModelWrapperIndividual(evalmodel_cfg, device)
    log_file = args.out
    evaluation(log_file)
