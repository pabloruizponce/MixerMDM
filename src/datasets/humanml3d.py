...
class HumanML3D(data.Dataset):
    ...
    HumanML3D dataset
    """
    def __init__(self, opt, extended=False, num_samples=-1):
        ...
        # Load data
        index = 0
        motion_path = pjoin(opt.DATA_ROOT, "joints_vec/")
        for file in tqdm(os.listdir(motion_path)):
            ...
            text_path = motion_file_path.replace("joints_vec", "texts").replace("npy", "txt")
    ...
    def __getitem__(self, item):
        ...
        # Return the data
        if self.extended:
            return name, text, gt_motion1, gt_motion1, gt_length, text, text
        else:
